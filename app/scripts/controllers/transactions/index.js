...
export default class TransactionController extends EventEmitter {
  ...
  */
  ...
  async createCancelTransaction (originalTxId, customGasPrice) {
    ...
    if (originalTxMeta.transactionCategory === TRANSACTION_CATEGORY_SWAP) {
  ...
  */
  async publishTransaction (txId, rawTx) {
    const txMeta = this.txStateManager.getTx(txId)
    txMeta.rawTx = rawTx
    if (txMeta.transactionCategory === TRANSACTION_CATEGORY_SWAP) {
  ...
  async confirmTransaction (txId, txReceipt) {
    ...
    try {
      ...
      if (initialTxMeta.transactionCategory === TRANSACTION_CATEGORY_SWAP) {
